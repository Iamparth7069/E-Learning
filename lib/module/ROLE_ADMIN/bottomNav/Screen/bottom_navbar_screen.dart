...
class DeskBoardScreenAdmin extends StatelessWidget {
  const DeskBoardScreenAdmin({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: AppColor.whiteColor,
      body: GetBuilder<AdminNavController>(
        init: AdminNavController(),
        builder: (controller) {
          return PageStorage(bucket: controller.bucket, child: controller.screens[controller.pageIndex].screen,);
        },),
      bottomNavigationBar: GetBuilder<AdminNavController>(
        init: AdminNavController(),
        builder: (controller) {
          return Container(
  ...
  List<Widget> _getBottomWidget(AdminNavController controller) {
    List<Widget> list = [];
